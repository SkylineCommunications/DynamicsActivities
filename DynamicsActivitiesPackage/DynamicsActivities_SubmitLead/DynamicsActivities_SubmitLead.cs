namespace DynamicsActivitiesSubmitLead
{
	using System;
	using System.Net;
	using System.Text;
	...
	/// <summary>
	/// Receives a lead submitted from the Dynamics Activities web app and emails it
	/// to the configured recipient. Called by the frontend via ExecuteAutomationScript.
	/// Parameters: Payload (JSON lead fields), UserEmail, UserName, Recipient (optional override).
	/// </summary>
	public class Script
	{
		...
		private void RunSafe(IEngine engine)
		{
			...
			{
				engine.ExitFail("Missing lead payload.");
				return;
			}
			...
			try
			{
				lead = JObject.Parse(payloadRaw);
			}
			catch (Exception ex)
			{
				engine.ExitFail("Invalid lead payload JSON: " + ex.Message);
				return;
			}

			var validationError = Validate(lead);
			...
			var body = BuildEmailBody(lead, userName, userEmail);

			engine.SendEmail(body, subject, recipient);
			engine.GenerateInformation($"[SubmitLead] Lead email sent to {recipient} (submitted by {userName} <{userEmail}>).");

			engine.AddScriptOutput("result", "{\"success\":true}");
		}

		private static string Validate(JObject lead)
		{
			if (string.IsNullOrWhiteSpace(Field(lead, "topic")))
			{
				return "Topic is required.";
			}

			if (string.IsNullOrWhiteSpace(Field(lead, "firstName")) || string.IsNullOrWhiteSpace(Field(lead, "lastName")))
			{
				return "First and last name are required.";
			}

			if (string.IsNullOrWhiteSpace(Field(lead, "company")))
			{
				return "Company is required.";
			}

			if (!IsValidEmail(Field(lead, "email")))
			{
		...
		private static string BuildSubject(JObject lead)
		{
			var name = string.Join(" ", new[] { Field(lead, "firstName"), Field(lead, "lastName") }).Trim();
			var company = Field(lead, "company");
			var who = !string.IsNullOrWhiteSpace(name) ? name : "Unknown contact";
		...
		private static string BuildEmailBody(JObject lead, string submitterName, string submitterEmail)
		{
			...
			sb.AppendLine("<div style='font-size:22px;font-weight:700;color:#151a22;'>New Lead</div>");
			...
			AppendRow(sb, "Topic", Field(lead, "topic"));
			AppendRow(sb, "First name", Field(lead, "firstName"));
			AppendRow(sb, "Last name", Field(lead, "lastName"));
			AppendRow(sb, "Company / Account", Field(lead, "company"));
			AppendRow(sb, "Job title", Field(lead, "jobTitle"));
			AppendRow(sb, "Email", Field(lead, "email"));
			AppendRow(sb, "Phone", Field(lead, "phone"));
			AppendRow(sb, "Country", Field(lead, "country"));
			AppendRow(sb, "Description", Field(lead, "description"));
		...
		private static string Field(JObject lead, string name)
		{
			return lead[name]?.Value<string>()?.Trim() ?? string.Empty;
		}
